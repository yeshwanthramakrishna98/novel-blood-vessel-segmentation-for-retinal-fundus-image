...
def calC_accuracy(result, label):
    ...
    print("Dice Coefficient =",dice(result ,label))
...
def extract_image(image):		
    ...
    #cv2.imshow("green",green_extractimg)
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
    
    enhanced_greenimg = clahe.apply(green_extractimg)
    cv2.imshow("After AHE",enhanced_greenimg)
...
if __name__ == "__main__":	
  ...
  for img,man in zip(glob.glob("DRIVE/test/images/*.tif"),glob.glob("DRIVE/test/test1/*.tiff")):
    ...
    pr=extract_image(image)
    h, w = pr.shape[:2]
    mask = np.zeros((h+2, w+2), np.uint8)
 
    
    cv2.floodFill(pr, mask, (270,2), 0);

    cv2.imshow("pr",pr)
    
    
    path = 'DRIVE/test/modified/'
    cv2.imwrite(str(path)+str(i)+'.tiff',pr)
    res=cv2.imread(man)
    res = cv2.cvtColor(res, cv2.COLOR_BGR2GRAY)
    #cv2.imshow("res",res)
    #print(res.shape)
    i=i+1
    p=calC_accuracy(pr, res)    
